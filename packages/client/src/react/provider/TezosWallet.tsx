...
export function TezosWallet(props: TezosWalletProps) {
  ...
  const handleAccountSetRef = useRef(handleAccountSet)

  useEffect(() => {
    handleAccountSetRef.current = handleAccountSet
  }, [handleAccountSet])

  useEffect(() => {
    invariant(config.beaconWallet, "BeaconWallet is required")
    const { beaconWallet } = config
    const handleEvent = (acc: AccountInfo | undefined) =>
      handleAccountSetRef.current(acc)
    beaconWallet.client.subscribeToEvent(
      BeaconEvent.ACTIVE_ACCOUNT_SET,
      handleEvent
    )
    // We manually retrieve the active account on mount
    beaconWallet.client.getActiveAccount().then(handleEvent)
  }, [])
