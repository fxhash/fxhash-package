 ...
 */
export class SocialWalletsFrameManager extends IframeBDCommHost<TMessages> {
  _config: SocialWalletsFrameConfig
  _container: HTMLElement
  _wrapper: HTMLDivElement | null = null
  _iframe: HTMLIFrameElement | null = null

  constructor(config: SocialWalletsFrameConfig) {
    super()
    //
    invariant(
      isBrowser(),
   ...
   * - iframe (& load the URL)
   */
  private initDOM(): PromiseResult<void, SocialWalletsFrameNotLoading> {
    return new Promise<Success<void>>((resolve, reject) => {
      ...
      this._iframe.onerror = err => {
        reject(failure(new SocialWalletsFrameNotLoading(this._config.url, err)))
      }
  ...
  async init(): PromiseResult<void, SocialWalletsFrameInitializationError> {
    ...
    // initialize the communication protocol
    {
      ...
      if (res.isFailure())
        return failure(new SocialWalletsFrameNotResponding(this._config.url))
    }
