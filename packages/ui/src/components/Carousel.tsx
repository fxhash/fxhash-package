...
interface CarouselThumbsProps<T> {
  items: T[]
  renderThumb: (item: T, index: number, isSelected: boolean) => React.ReactNode
  className?: string
...
function CarouselThumbs<T>({
  ...
  React.useEffect(() => {
    if (!api || !emblaThumbsApi) return
    api.on("select", handleOnSelect)
    return () => {
      api.off("select", handleOnSelect)
    }
  }, [api, emblaThumbsApi])
  ...
  return (
    <div ref={emblaThumbsRef} className="overflow-hidden">
      <div
        className={cn("mt-6 flex items-center justify-start gap-4", className)}
      >
        {items.map((item, index) => (
          ...
          >
            {renderThumb(item, index, selectedIndex === index)}
          </button>
        ))}
      </div>
    </div>
