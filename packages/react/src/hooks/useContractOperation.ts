...
import { NoWalletConnectedForNetworkError, RichError } from "@fxhash/errors"
...
import { Failure, IEquatableError, PromiseResult, Success } from "@fxhash/utils"
import { TEthereumContractOperation } from "@fxhash/eth"
import { useEvmWalletManager } from "./useEvmWallet"
...
export function useContractOperation<
  ...
  TInput = TOperations[TBlockchain] extends TEthereumContractOperation<infer P>
    ? P
    : never,
>(
  ...
  const ethereumWalletManager = useEvmWalletManager()
  ...
  const execute = useCallback(
    async <TBlockchain extends BlockchainType>(
      ...
      try {
        const walletManager = ethereumWalletManager
        if (!walletManager) {
          setState(() => ({
            status: ContractOperationStatus.ERROR,
            called: false,
    ...
    [operations, ethereumWalletManager]
  )
  ...
  const rawExecute = useCallback(
    async <TBlockchain extends BlockchainType>(
      ...
      const walletManager = {
        ...
        [BlockchainType.TEZOS]: ethereumWalletManager,
      }[blockchainType]
