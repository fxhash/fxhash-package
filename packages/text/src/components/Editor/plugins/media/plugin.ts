import { arrayRemoveDuplicates } from "@fxhash/utils"
import {
  BaseOperation,
  Editor,
  Transforms,
...
import { isFormatActive } from "../../utils/isFormatActive.js"
...
function insertImage(editor: Editor, url: string) {
  Transforms.insertNodes(
    editor,
    editor.getBlockDefinition("image").instanciateElement!({ url, caption: "" })
  )
}
function insertVideo(editor: Editor, src: string) {
  Transforms.insertNodes(
    editor,
    editor.getBlockDefinition("video").instanciateElement!({ src, caption: "" })
  )
}
function insertAudio(editor: Editor, src: string) {
  Transforms.insertNodes(
    editor,
    editor.getBlockDefinition("audio").instanciateElement!({ src, caption: "" })
  )
}
 ...
 * node
 */
function getEditorMedias(editor: FxTextEditor): FxTextEditorMediaFile[] {
  const medias: FxTextEditorMediaFile[] = []
