...
import { customElements } from "@/processor/_index"
...
function parseAttributes(attributes: string | undefined): {
  ...
  entries.forEach(entry => {
    if (entry.startsWith("#")) {
      ...
      const keyValueMatcher = new RegExp(
        '"*(?<key>.*)"*="*(?<value>[^"]*)"*',
        "mg"
      )
...
export class CustomDirectiveChange implements AutoFormatChange {
  ...
  apply(editor: Editor): boolean {
    ...
    const matchDirective = new RegExp(
      "(:*s*(?<type>[^\\s]*)\\s*\\[(?<text>.*)]\\s*{(?<attributes>.*)})",
      "mg"
    )
    ...
    const props =
      customElements.leafDirective[type]?.transformMdhastToComponent?.(
        null as any,
        nodeAttributes
      ) || nodeAttributes
